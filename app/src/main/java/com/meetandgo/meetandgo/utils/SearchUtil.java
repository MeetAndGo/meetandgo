...
public final class SearchUtil {
    ...
    //TODO: Check if calculate Search Works

    public void calculateSearch(Map<String,Object> searches, Search userSearch){
        List<Search> results = new ArrayList<Search>();
        double[][] resultOrder = new double[results.size()][2];
        int count = 0;
        for(Map.Entry<String, Object> entry : searches.entrySet())
        {
            ...
            results.add(search);
            //PROCESS SEARCH
            if(search.getUserId() != userSearch.getUserId()) {
                //Check Preference
